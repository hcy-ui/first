...
uint8_t USART2_Serial_RxData;		//定义串口接收的数据变量
uint8_t USART2_Serial_RxFlag;		//定义串口接收的标志位变量
...
void USART2_Serial_SendArray(uint8_t *Array, uint16_t Length)
{
	...
	{
		USART2_Serial_SendByte(Array[i]);		//依次调用Serial_SendByte发送每个字节数据
	}
}
...
void USART2_Serial_SendString(char *String)
{
	...
	{
		USART2_Serial_SendByte(String[i]);		//依次调用Serial_SendByte发送每个字节数据
	}
}
...
/**
  ...
  * 返 回 值：返回值等于X的Y次方
  */
uint32_t USART2_Serial_Pow(uint32_t X, uint32_t Y)
{
...
void USART2_Serial_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	{
		USART2_Serial_SendByte(Number / USART2_Serial_Pow(10, Length - i - 1) % 10 + '0');	//依次调用Serial_SendByte发送每位数字
	}
}
...
{
	USART2_Serial_SendByte(ch);			//将printf的底层重定向到自己的发送字节函数
	return ch;
}
...
void USART2_Serial_Printf(char *format, ...)
{
	...
	va_end(arg);					//结束变量arg
	USART2_Serial_SendString(String);		//串口发送字符数组（字符串）
}
...
{
	if (USART2_Serial_RxFlag == 1)			//如果标志位为1
	{
		USART2_Serial_RxFlag = 0;
...
{
	return USART2_Serial_RxData;			//返回接收的数据变量
}
...
void USART2_IRQHandler(void)
{
	...
	{
		USART2_Serial_RxData = USART_ReceiveData(USART2);				//读取数据寄存器，存放在接收的数据变量
		USART2_Serial_RxFlag = 1;										//置接收标志位变量为1
