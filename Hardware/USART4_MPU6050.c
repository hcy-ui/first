...
uint8_t UART4_Serial_RxData;		//定义串口接收的数据变量
uint8_t UART4_Serial_RxFlag;		//定义串口接收的标志位变量
...
void UART4_Serial_SendArray(uint8_t *Array, uint16_t Length)
{
	...
	{
		UART4_Serial_SendByte(Array[i]);		//依次调用Serial_SendByte发送每个字节数据
	}
}
...
void UART4_Serial_SendString(char *String)
{
	...
	{
		UART4_Serial_SendByte(String[i]);		//依次调用Serial_SendByte发送每个字节数据
	}
}
...
/**
  ...
  * 返 回 值：返回值等于X的Y次方
  */
uint32_t UART4_Serial_Pow(uint32_t X, uint32_t Y)
{
...
void UART4_Serial_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	{
		UART4_Serial_SendByte(Number / UART4_Serial_Pow(10, Length - i - 1) % 10 + '0');	//依次调用Serial_SendByte发送每位数字
	}
}
...
/**
  ...
  * 返 回 值：保持原始格式即可，无需变动
  */
int UART4_fputc(int ch, FILE *f)
{
	UART4_Serial_SendByte(ch);			//将printf的底层重定向到自己的发送字节函数
	return ch;
}
...
void UART4_Serial_Printf(char *format, ...)
{
	...
	va_end(arg);					//结束变量arg
	UART4_Serial_SendString(String);		//串口发送字符数组（字符串）
}
...
{
	if (UART4_Serial_RxFlag == 1)			//如果标志位为1
	{
		UART4_Serial_RxFlag = 0;
...
{
	return UART4_Serial_RxData;			//返回接收的数据变量
}
...
void UART4_IRQHandler(void)
{
	...
	{
		UART4_Serial_RxData = USART_ReceiveData(UART4);				//读取数据寄存器，存放在接收的数据变量
		UART4_Serial_RxFlag = 1;										//置接收标志位变量为1
